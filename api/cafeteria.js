// api/cafeteria.js — Evaluaciones cafetería + reporte semanal
...
function semanasFiltro(evs, semanas) {
  const corte = new Date(); corte.setDate(corte.getDate() - semanas * 7);
...
export default async function handler(req, res) {
  ...
  try {
    // ── Reporte semanal (cron o llamada autorizada) ──
    if (req.method === 'GET' && req.query?.accion === 'reporte') {
...
<div style="max-width:600px;margin:30px auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08)">
  ...
  <div style="padding:28px 32px">
    ...
    ${bloqueperiodo('Última semana',    semanasFiltro(evs,1))}
    ${bloqueperiodo('Últimas 3 semanas',semanasFiltro(evs,3))}
    ${bloqueperiodo('Últimas 5 semanas',semanasFiltro(evs,5))}
    ${bloqueperiodo('Últimas 10 semanas',semanasFiltro(evs,10))}
  </div>
  <div style="background:#f8fafc;padding:18px 32px;border-top:1px solid #e2e8f0;text-align:center">
    <p style="margin:0;font-size:12px;color:#94a3b8">RecreaBot · Instituto Superior Neumann ·
